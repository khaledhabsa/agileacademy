...
//        $this->args['tab_prefix'] = 'et_'.self::$option_name;
    ...
    public function add_custom_field_variation_data($variations) {
        if ( count($this->settings)) {
            if ( $this->settings['custom_values_for_statuses'] ) {
                ...
                if ( $stock_status && $local_settings['separated_values_for_'.$stock_status]) {
    ...
    public function set_settings($custom_settings = array()) {
        ...
        foreach (array_keys($estimated_delivery_only_for) as $estimated_delivery_only_for_item ) {
            if ( $local_settings['separated_values_for_'.$estimated_delivery_only_for_item] )
        ...
        foreach ( array_merge(array('text_before', 'min_days', 'max_days', 'days' ), $estimated_delivery_product_extra_options) as $product_custom_option_key ) {
            ...
            if ( !$local_settings['custom_values_for_statuses'] && in_array($product_custom_option_key, $estimated_delivery_product_extra_options) && isset($product_custom_options[$product_custom_option_key])) {
        ...
        if ( (!$this->args['is_admin'] || $this->args['in_quick_view']) && isset($this->args['product']) && $this->args['product'] ) {
            ...
            if ( $this->args['should_render'] ) {
                ...
                foreach (array_keys($estimated_delivery_only_for) as $status) {
                    if ($this->settings['separated_values_for_' . $status] && $this->args['product']->get_stock_status() == $status) {
    ...
    public function output() {
        ...
        <<?php echo esc_attr($this->args['tag']) ?> class="sales-booster-estimated-delivery">
            <?php echo esc_html($settings['text_before']) . $this->calculated_date($settings); ?>
...
//		}
            ...
            <div class="options_group">
                <p class="form-field">
                    ...
                    if ( $this->settings['date_type'] == 'range' ) {
                        ...
                        if ( $this->settings['custom_values_for_statuses'] ) {
                            foreach ($stock_statuses as $stock_status_key => $stock_statuses_title) {
                                woocommerce_wp_text_input(
                                    array(
                                        'id' => $this->args['tab_prefix'] . '_min_days_' . $stock_status_key,
                                        'label' => sprintf(esc_html__('Min days for "%s" status', 'xstore'), $stock_statuses_title),
                                ...
                                woocommerce_wp_text_input(
                                    array(
                                        ...
                                        'label' => sprintf(esc_html__('Max days for "%s" status', 'xstore'), $stock_statuses_title),
                            ...
                            foreach ($stock_statuses as $stock_status_key => $stock_statuses_title) {
                                woocommerce_wp_text_input(
                                    array(
                                        ...
                                        'label' => sprintf(esc_html__('Days for %s status', 'xstore'), $stock_statuses_title),
    ...
    public function save_panel_data( $post_id ) {
        ...
        foreach (array_keys($stock_statuses) as $stock_status_key) {
