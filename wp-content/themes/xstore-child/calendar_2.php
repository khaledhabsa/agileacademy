...
foreach ($query->get_posts() as $post) {
    $event = array(
        ...
        'duration' => get_field('Duration', $post->ID), // instead of startTime and end time
...
<body class="box-border" x-data="{ showCalendar: false }">
            ...
            <div class="flex-container row-md justify-between" x-show="!showCalendar">
                <div class="button-image-container md-margin-right-40 margin-right-5">
                    <button id="currentRange" data-dropdown-toggle="dropdownNavbar6" class="btn">
                        <!-- Your button content -->
                        4 Mar, 2022 - 9 May, 2022 <svg class="w-2.5 h-2.5 ml-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
                ...
                <?php foreach ($query->get_posts() as $post) : ?>
                    <div class="custom-course">
                        ...
                        <div class="custom-card">
                            <img src="<?= get_the_post_thumbnail_url($post->ID) ?>" class="custom-image">
                            <div class="custom-content-course">
                                <p class="custom-title"><?= $post->post_title ?></p>
                                <p class="custom-description"><?= $post->post_content ?></p>
    ...
    <script>
        ...
        const events = <?php echo json_encode($events); ?>; // calendar events
        ...
        // Get all the items in the list (assuming they have a common class)
        const listItems = document.querySelectorAll('.custom-course');

        // Add an event listener to the search input field
        searchInputlist.addEventListener('input', function() {
            const searchTerm = searchInputlist.value.toLowerCase();

            // Loop through all list items and hide/show based on the search term
            listItems.forEach(item => {
                const itemTitle = item.querySelector('.custom-title').textContent.toLowerCase();
                const itemDescription = item.querySelector('.custom-description').textContent.toLowerCase();
